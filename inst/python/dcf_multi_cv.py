...
for o in range(n_outcomes):
    ...
    if sum(row_bool) >= 10:
        for j in range(n_causes):
            ...
            y = Y[row_bool, o]
            reg_no_ctrl = fit_outcome_linear(G[row_bool,j][:,np.newaxis], y, 1, CV=True, verbose=True)
            pmf_no_ctrl[j][o] = reg_no_ctrl.coef_
            reg_dcf = fit_outcome_linear(X, y, 1, CV=True, verbose=True)
            pmf_dcf[j][o] = reg_dcf.coef_[0]
    else:
        print("Skip outcome {}".format(o))


np.savetxt(os.path.join(out_dir, "pmf_no_ctrl_indept_coef_cv.txt"), pmf_no_ctrl)
np.savetxt(os.path.join(out_dir, "pmf_dcf_indept_coef_cv.txt"), pmf_dcf)

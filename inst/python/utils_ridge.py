...
def fit_outcome_linear(X, y, n_causes, regularization="Ridge", params=np.logspace(-3, 3, 7), true_betas=None, CV=True, verbose=True):
    if CV == True:
        linear_reg = regression_CV(X, y, regularization, params, outtype="linear", verbose=verbose)
    else:
        linear_reg = regression_noCV(X, y, regularization, outtype="linear", verbose=verbose)
    if true_betas!= None:
        linear_rmse = np.sqrt(((true_betas - linear_reg.coef_[:n_cause])**2).mean())
...
def fit_outcome_logistic(X, y_bin, n_causes, regularization="Ridge", params=np.logspace(-3, 3, 7), true_betas=None, CV=True, verbose=True):
    if CV == True:
        logistic_reg = regression_CV(X, y_bin, regularization, params, outtype="logistic", verbose=verbose)
    else:
        logistic_reg = regression_noCV(X, y_bin, regularization, outtype="logistic", verbose=verbose)
    if true_betas!=None:
...
def regression_CV(X, y, regularization, params, outtype="linear", verbose=True):
    ...
    if outtype == "linear":
        ...
        if regularization == "Ridge":
            ...
            print("Only Ridge regularization is supported for binary outcome.")

    parameters = dict(alpha=params)
    ...
    if outtype == "linear":
        if regularization == "Ridge":
            ...
            reg = linear_model.Lasso(alpha=bestalpha)
    elif outtype == "logistic":
        if regularization == "Ridge":
...
def regression_noCV(X, y, regularization, outtype="linear", verbose=False):
    X_train, X_test, y_train, y_test = \
        train_test_split(X, y, test_size=0.3, random_state=123)
    ...
    if outtype == "linear":
        ...
        if regularization == "Ridge":
            reg = linear_model.RidgeClassifier(alpha=1000.)
        else:
            print("Only Ridge regularization is supported for binary outcome.")

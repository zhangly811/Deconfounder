...
SELECT 0 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
                                           (
                                             ...
                                             and ca.ancestor_concept_id in (@ingredient_concept_ids)
...
SELECT 1 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
                                           (
                                             select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (@measurement_concept_ids)
...
(
  ...
  (
    ...
    (
      ...
      WHERE DATEADD(day,1,OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,1,E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
    ) P
    ...
    JOIN (
      ...
      (
        ...
        (
          ...
          ) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-@lab_window,P.START_DATE) AND A.START_DATE <= DATEADD(day,-1,P.START_DATE)
    ...
    JOIN (
      ...
      (
        ...
        (
          ...
          ) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,1,P.START_DATE) AND A.START_DATE <= DATEADD(day,@lab_window,P.START_DATE)
          GROUP BY p.person_id, p.event_id
          HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
          -- End Correlated Criteria
        ...
        (
          ...
          (
            ...
            (
              ...
              (
                --cteEndDates
                select PERSON_ID, DATEADD(day,-1 * @drug_window,EVENT_DATE) as END_DATE -- unpad the end date by 60
                FROM
                (
                  ...
                  (
                    ...
                    select PERSON_ID, DATEADD(day,@drug_window,DRUG_EXPOSURE_END_DATE), 1 as EVENT_TYPE, NULL
                    FROM #drugTarget D
                  ) RAWDATA
                ) E
